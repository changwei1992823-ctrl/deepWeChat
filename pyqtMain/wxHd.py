	...
	"""
{
  'result': [{
	...
	def getHeart(self):
		url = "http://127.0.0.1:8888/heart"
		json2 = {"msg": "WeChatApis is running......"}
		data = web.post(url, [], json2)
		print("getHeart", data)
		pass

	# 攻击点我，谢谢 然后推送名片
	def sendGongJiDianWo(self, withWo=True):
		if config["set_1"]["buJd"]:
			return
		# print("sendGongJiDianWo")
		text = utils.vsMsg("gongJiDianWo")
		if withWo:
			self.sendTextMsg(text, "", "gongJiDianWo")
			self.sendMingPian()
		else:
			self.tuiMingPian()
	...
	def tuiMingPian(self, to_wxid = ""):
		if to_wxid == "":
			...
			if self.cbx_2_1.currentIndex() > -1:
				to_wxid = self.getJieDanQun1()
		card_wxid = config["ads"]["tid"]
		card_name = config["ads"]["ttx"]
		# card_wxid = "youchangwei"
		# card_name = "card_name"
		if to_wxid != "" and hasattr(self, "myWxMsg") and self.myWxMsg:
			...
			if config["set_1"]["hookMd"] == 1:
				...
				url = GM["baseApi"] + "send_card_msg"
			try:
			# if True:
				data = web.post(url, [], json2)
				# print("tuiMingPian.data", data)
				if data.status_code == 200:
					mJson = json.loads(data.text)
					# print("sendMingPian", mJson)
	...
	def sendMingPian(self, to_wxid = ""):
		...
		if to_wxid != "" and hasattr(self, "myWxMsg") and self.myWxMsg:
			...
			json2 = {
				"type": 6,
				"to_wxid": to_wxid,
				"card_wxid": self.myWxMsg["wxid"], # 这里要用 wxcount 而非 wxid
			...
			# if True:
				data = web.post(url, [], json2)
				# print("sendMingPian.data", data, data.text)
				if data.status_code == 200:
					mJson = json.loads(data.text)
					# print("sendMingPian", mJson)
					# mJson-{'result': '请求已经被成功处理'}
